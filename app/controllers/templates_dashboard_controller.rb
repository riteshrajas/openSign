...
class TemplatesDashboardController < ApplicationController
  ...
  def filter_templates(templates)
    ...
    if params[:q].blank?
      if OpenSeal.multitenant? ? current_account.testing? : current_account.linked_account_account
        shared_account_ids = [current_user.account_id]
        shared_account_ids << TemplateSharing::ALL_ID if !OpenSeal.multitenant? && !current_account.testing?
